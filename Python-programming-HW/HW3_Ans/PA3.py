...
def drop(*moves, player='Auto', toggle=True, **kws):
    ...
    if 'reset' in kws:
        ...
        if B[0][0]!="  " and B[1][0]!="  " and B[2][0]!="  " and B[3][0]!="  " and B[4][0]!="  " and B[5][0]!="  " and B[6][0]!="  ":
            print("The board is full, so %c cannot randomly place."%(token))
            return False
        else:
            c=rnd(1,7,1)
            while B[c-1][0]!="  ":
            ...
            while B[c-1][i]!="  ":
                i-=1
            if 'announce' in kws:
                print("%c places into column %d."%(token,c))
    else:
        for x in range(n):
            if B[0][0]!="  " and B[1][0]!="  " and B[2][0]!="  " and B[3][0]!="  " and B[4][0]!="  " and B[5][0]!="  " and B[6][0]!="  ":
                print("The board is full, so %c cannot randomly place."%(token))
            ...
            if 'announce' in kws:
                print("%c places into column %d."%(token,c))
            if toggle==True and player=='Auto':
                if token=='⚫':
                    token='⚪'
                elif token=='⚪':
                    token='⚫'
    ...
    B[c-1][i]=token
      ...
      #is there is a way to flip it around and let .index() work nonetheless?
    #
...
def display():
    ...
    for i in range(6):
        ...
        #        this:  "|⚪|  |⚪|⚪|  |  |⚫|"
        print("|", end='');print(Bzip[i][0],Bzip[i][1],Bzip[i][2],Bzip[i][3],Bzip[i][4],Bzip[i][5],Bzip[i][6],sep='|',end='|\n')
        #Line 5:
        if i==5: break
        #Line 6:
        print("├──┼──┼──┼──┼──┼──┼──┤")
