...
rnd = lambda x : randrange(0, x)
setAttribute = lambda obj, name, value : setattr(obj, name, value)
...
#     can do this even if it is the name of one of the function's parameters.
    ...
    def __init__(self, *args, joker=False, faceup=False, **kwlist):
        if(len(kwlist)!=0 or (len(args)!=0 and len(args)!=2)):
            raise SyntaxError
        elif(len(args)==0):
            rndNum = rnd(54) if(joker) else rnd(52)
            if(rndNum==53):
                self.value = 13
                self.suit = 1
            elif(rndNum==52):
                ...
                self.value = rndNum % 13
                self.suit = rndNum // 13
        else:
            if(args[0]=="joker"):
                self.value = 13
            else:
                self.value = Card.values.index(args[0][0].upper())
            if(args[1][0].upper()=='C'):
                self.suit = Card.suits.index('♣')
            elif(args[1][0].upper()=='D'):
                self.suit = Card.suits.index('♦')
            elif(args[1][0].upper()=='H'):
                self.suit = Card.suits.index('♥')
            elif(args[1][0].upper()=='S'):
                self.suit = Card.suits.index('♠')
        setAttribute(self, "faceup", faceup)        
...
# safe to hash it (ie to use these cards as elements of sets).
    ...
    def flip(self):
        setAttribute(self, "faceup", not self.faceup)
...
# Because I have not taught you about color codes, I'll give that part to you:
    def __str__(self):
        ...
        S += Card.values[self.value] + (Card.suits[self.suit] if(self.value!=13) else "") if(self.faceup) else "⠿⠿" 
        ...
        S += sub(Card.values[self.value] + (Card.suits[self.suit] if(self.value!=13) else ""), "  ", S)
...
#    bottom strings (along with adding some extra "\n" characters).
    ...
    def __add__(self, obj):
        add=self.__str__()+obj.__str__()
        add=[x+y for x, y in zip([add.split("\n")[i] for i in [0,1]], [add.split("\n")[i] for i in [2,3]])]
        return add[0] + "\n" + add[1]
