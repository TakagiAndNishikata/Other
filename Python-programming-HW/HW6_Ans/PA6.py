...
class Connect4():
    p=ChainMap(dict(zip_longest({'⚪','e','E','h','H','o','O','empty','hollow','open','Empty','Hollow','Open','EMPTY','OPEN','HOLLOW'},{},fillvalue='⚪')),dict(zip_longest({'⚫','c','C','f','F','closed','filled','full','fill','Closed','Filled','Full','Fill','FULL','FILL','CLOSED','FILLED'},{},fillvalue='⚫')))
...
###############################################################################
    def __init__(self,piece=None):
        ...
        self.isFilled=Connect4.token(piece) == '⚪' if piece!=None else True
...
###############################################################################
    def __setitem__(self,c,token):
        if c in self:
            self.B[c-1][5-[*reversed(self.B[c-1])].index('  ')]=token
        else:
            raise
